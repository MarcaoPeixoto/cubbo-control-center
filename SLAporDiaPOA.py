...
dt_ini = datetime.strptime('27/05/2025', '%d/%m/%Y')
dt_end = datetime.now()
...
def ajuste_pendentes(only_process_today=True, max_day_retries=5):
    ...
    if not only_process_today:
        ...
        while current_date <= end_date:
            # Skip Sundays and holidays for historical processing
            if current_date.weekday() < 6 and current_date.strftime('%Y-%m-%d') not in holidays:
        ...
        # Log if it's a Sunday or holiday, but still process
        if today.weekday() == 6:
            print(f"Note: Today ({today.strftime('%d/%m/%Y')}) is a Sunday, but still processing data")
...
def process_single_day(day_to_process, max_retries=5):
    ...
    while retry_count < max_retries:
        try:
            ...
            daily_orders = get_dataset('8201', incentivo_inputs)
            
            # Check if we got valid data back
            if not daily_orders or not isinstance(daily_orders, list):
                retry_count += 1
                wait_time = retry_count * 2  # Increasing wait time with each retry
...
def generate_csv(data, google_sheet_data, sheet_url, only_process_today=True):
                    ...
                    for record in existing_records:
                        ...
                        try:
                            ...
                            existing_data.append({
                                ...
                                'ITEMS_HIT': int(record.get('ITEMS HIT', 0)),
                                'ITEMS_MISS': int(record.get('ITEMS MISS', 0)),
                                'TOTAL_ITEMS_SHIPPED': int(record.get('TOTAL ITEMS SHIPPED', 0)),
                                'ITEMS_PENDING': int(record.get('ITEMS PENDING', 0)),
                                'ITEMS_SLA': float(record.get('ITEMS SLA (%)', 0)),
                                'ITEMS_PER_OPERATOR': float(record.get('ITEMS PER OPERATOR', 0))
                            })
                        except Exception as e:
    ...
    # Initialize date_summary with existing data first
        ...
        for record in existing_data:
            try:
                ...
                date_summary[date_str] = {
                    ...
                    'STILL_PENDING': record['PENDING'],  # Map old PENDING to new STILL_PENDING
                    'PENDING_ON_DAY': 0,  # Initialize new column
                    'TAG_MISS': 0,  # We don't have this from historical data
                    'ITEMS_HIT': record.get('ITEMS_HIT', 0),
                    'ITEMS_MISS': record.get('ITEMS_MISS', 0),
                    'TOTAL_ITEMS_SHIPPED': record.get('TOTAL_ITEMS_SHIPPED', 0),
                    'ITEMS_STILL_PENDING': record.get('ITEMS_PENDING', 0),  # Map old ITEMS_PENDING to new ITEMS_STILL_PENDING
                    'ITEMS_PENDING_ON_DAY': 0  # Initialize new column
                }
    ...
    # Dictionary to track orders by their status and date
    orders_tracking = {}

    # First pass: collect all orders and their status
    for entry in data:
        try:
            # Get number of items for this order
            num_items = entry.get('itens', 1)  # Default to 1 if not specified
            
            # Process pending date
            pending_date_str = entry['pending_at'].strftime('%d-%m-%Y')
            
            # Track the order with its details
            order_info = {
                'pending_date': pending_date_str,
                'items': num_items,
                'shipping_date': entry['shipping_date'] if entry['shipping_date'] != "PROCESSANDO" else None
            }
            
            # Store order info by its order number
            orders_tracking[entry['order_number']] = order_info

        except Exception as e:
            print(f"Error in first pass: {e}")
            continue

    # Second pass: process the data and update summaries
    for entry in data:
        try:
            # Get number of items for this order
            num_items = entry.get('itens', 1)  # Default to 1 if not specified
            ...
            if pending_date_str not in date_summary:
                date_summary[pending_date_str] = {
                    ...
                    'ITEMS_STILL_PENDING': 0,
                    'ITEMS_PENDING_ON_DAY': 0
                }
            
            # Count orders and items that entered on this day (by pending_at date)
            date_summary[pending_date_str]['PENDING_ON_DAY'] += 1
            date_summary[pending_date_str]['ITEMS_PENDING_ON_DAY'] += num_items
            
            # Process shipping date data if available
            if entry['shipping_date'] != "PROCESSANDO":
                ...
                # Count as shipped (both orders and items)
                date_summary[shipping_date_str]['TOTAL_SHIPPED'] += 1
                date_summary[shipping_date_str]['TOTAL_ITEMS_SHIPPED'] += num_items
                
                # Update SLA counters (both orders and items)
                if entry['SLA'] == 'HIT':
                    date_summary[shipping_date_str]['HIT'] += 1
                    date_summary[shipping_date_str]['ITEMS_HIT'] += num_items
                elif entry['SLA'] == 'MISS':
                    ...
                    if entry['store_name'] == "TAG Livros":
                        ...
                        date_summary[pending_date_str]['ITEMS_MISS'] += num_items
    ...
    all_dates = sorted(set(date_summary.keys()), key=lambda x: datetime.strptime(x, '%d-%m-%Y'))

    # For each date, calculate pending orders and daily shipments
    for i, current_date in enumerate(all_dates):
        ...
        # Count shipped orders for this specific date
        daily_shipped = 0
        daily_items_shipped = 0
        
        # Check each order
        for order_info in orders_tracking.values():
            order_pending_date = datetime.strptime(order_info['pending_date'], '%d-%m-%Y')
            
            # Count pending orders (orders entered on or before this date that aren't shipped yet)
            if order_pending_date <= current_date_obj:
                # If order is still processing or was shipped after current date
                if (order_info['shipping_date'] is None or 
                    (isinstance(order_info['shipping_date'], datetime) and 
                     order_info['shipping_date'].strftime('%d-%m-%Y') > current_date)):
                    pending_orders += 1
                    pending_items += order_info['items']
            
            # Count orders shipped on this specific date
            if (isinstance(order_info['shipping_date'], datetime) and 
                order_info['shipping_date'].strftime('%d-%m-%Y') == current_date):
                daily_shipped += 1
                daily_items_shipped += order_info['items']
        
        # Update the summary for this date
        date_summary[current_date]['STILL_PENDING'] = pending_orders
        date_summary[current_date]['ITEMS_STILL_PENDING'] = pending_items
        date_summary[current_date]['TOTAL_SHIPPED'] = daily_shipped
        date_summary[current_date]['TOTAL_ITEMS_SHIPPED'] = daily_items_shipped

    # Sort dates chronologically
    sorted_dates = sorted(date_summary.keys(), key=lambda x: datetime.strptime(x, '%d-%m-%Y'))

    # Prepare sheet data with both order and item metrics
    new_sheet_data = []
    new_sheet_data.append([
        'DATA',
        'PEDIDOS NO PRAZO',
        'ITENS NO PRAZO',
        'PEDIDOS ENVIADOS EM ATRASO',
        'PEÇAS ENVIADAS EM ATRASO',
        'ENVIOS TOTAL',
        'PEÇAS ENVIADOS TOTAL',
        'NÃO ENVIADOS',
        'PEÇAS NÃO ENVIADAS',
        'PENDENTES DO DIA',
        'PEÇAS PENDENTES DO DIA',
        'SLA (%)',
        'PEÇAS SLA (%)',
        'ENVIOS POR OPERADOR',
        'PEÇAS POR OPERADOR'
    ...
    for date in sorted_dates:
        try:
            ...
            # Calculate order-based SLA percentage (excluding TAG Livros misses)
            total_for_sla = summary['HIT'] + summary['MISS']
            sla_percentage = round((summary['HIT'] / total_for_sla * 100), 2) if total_for_sla > 0 else 0
            
            # Calculate item-based SLA percentage
            total_items_for_sla = summary['ITEMS_HIT'] + summary['ITEMS_MISS']
            items_sla_percentage = round((summary['ITEMS_HIT'] / total_items_for_sla * 100), 2) if total_items_for_sla > 0 else 0
            ...
            # Calculate shipments and items per operator
            shipments_per_operator = round(summary['TOTAL_SHIPPED'] / operators, 2)
            items_per_operator = round(summary['TOTAL_ITEMS_SHIPPED'] / operators, 2)
            ...
            new_sheet_data.append([
                ...
                summary['TOTAL_ITEMS_SHIPPED'],
                summary['STILL_PENDING'],
                summary['ITEMS_STILL_PENDING'],
                summary['PENDING_ON_DAY'],
                summary['ITEMS_PENDING_ON_DAY'],
                sla_percentage,
                items_sla_percentage,
    ...
    headers = new_sheet_data[0]
    column_widths = []
    for header in headers:
        # Calculate width based on header length (minimum 100 pixels)
        width = max(100, len(header) * 10)  # Approximately 10 pixels per character
        column_widths.append({"pixelSize": width})

    # Apply column widths
    worksheet.set_basic_filter()  # Add filter to header row
    worksheet.format("A1:Z1", {"textFormat": {"bold": True}})  # Make headers bold
    requests = [{
    ...
    print(f"Data has been written to the '{new_sheet_name}' tab in the Google Sheet with adjusted column widths.")
...
if __name__ == "__main__":
    ...
    google_sheet_url = 'https://docs.google.com/spreadsheets/d/1EOHwVmOQT-pE4-1yaadYGlJ2CM3KWYEPvJ0a-UuSnNw/edit?gid=0#gid=0'
